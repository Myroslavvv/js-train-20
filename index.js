...
function Truck(
  ...
  Truck.prototype.specific = function(weight) {
    if(weight > towingCapacity) {
      console.log("Навантаження занадто важке для буксирування 15000")
    } else {
      console.log("Тягнення навантаження...8000")
    }
  };
}
...
let truck1 = new Truck("Toyota", "Tundra", 2019, 20000, "Red", "V8", 10000, "Gasoline", "Automatic", 4, 5600);

truck1.specific(8000);
truck1.specific(15000);

Car.prototype.drive = function(item) {
  this.kilometers += item;
  console.log(`Подорожуємо ${item} кілометрів у ${this.brand} ${this.model}`)
};
let res = Car.prototype.drive.bind(truck1);
res(100);
